...
<#
    .SYNOPSIS
        Script to export the Group Members of the selected Azure AD groups to a CSV file.

    .DESCRIPTION
        This script will create a CSV report which will have the selected Azure AD groups and their member details.
        This script requires the Microsoft.Graph powershell module and the command to install this module is already mentioned.
...
param(
    [Parameter(Mandatory,
      HelpMessage = "Enter the pattern for filtering the groups"
    )]
    $Pattern
)
# CSV file path
...
$Groups = Get-MgGroup -Filter "startswith(displayName,'$Pattern')" 
