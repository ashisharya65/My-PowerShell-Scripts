...
# Generated at: 05/07/2024 14:34:27
...
PROCESS {
    ...
    if ( $__boundParameters["Verbose"]) {
         Write-Verbose -Verbose -Message "C:\Program Files\Docker\Docker\resources\bin\docker.exe"
    ...
    if ( $PSCmdlet.ShouldProcess("C:\Program Files\Docker\Docker\resources\bin\docker.exe $__commandArgs")) {
    # check for the application and throw if it cannot be found
        if ( -not (Get-Command -ErrorAction Ignore "C:\Program Files\Docker\Docker\resources\bin\docker.exe")) {
          throw "Cannot find executable 'C:\Program Files\Docker\Docker\resources\bin\docker.exe'"
        }
        if ( $__handlerInfo.StreamOutput ) {
            if ( $null -eq $__handler ) {
                & "C:\Program Files\Docker\Docker\resources\bin\docker.exe" $__commandArgs
            }
            else {
                & "C:\Program Files\Docker\Docker\resources\bin\docker.exe" $__commandArgs 2>&1| Push-CrescendoNativeError | & $__handler
            }
        }
        else {
            $result = & "C:\Program Files\Docker\Docker\resources\bin\docker.exe" $__commandArgs 2>&1| Push-CrescendoNativeError
            & $__handler $result
        }
    }
...
Global Options:
  ...
  -l, --log-level string   Set the logging level ("debug", "info",
      ...
      --tlscacert string   Trust certs signed only by this CA (default
                           "C:\\Users\\L1085665\\.docker\\ca.pem")
      --tlscert string     Path to TLS certificate file (default
                           "C:\\Users\\L1085665\\.docker\\cert.pem")
      --tlskey string      Path to TLS key file (default
                           "C:\\Users\\L1085665\\.docker\\key.pem")
...
.DESCRIPTION See help for C:\Program Files\Docker\Docker\resources\bin\docker.exe

.PARAMETER all
List all the containers, including those that aren't running.
