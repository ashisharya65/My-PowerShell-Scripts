...
<#
    ...
    .DESCRIPTION
        With this Powershell script, we will get an MS excel report of all the Intune apps created in your Tenant and their assigned Azure AD groups.This script uses the MSAL.PS Powershell Module to get the access token in order to interact with various Microsoft Graph (a type of REST) API endpoints.
...
# Checking and Verifying if the latest ImportExcel PowerShell Module is installed 
$latest = Find-Module -Name 'ImportExcel' -AllVersions -AllowPrerelease | Select-Object -First 1
$current = Get-InstalledModule | Where-Object { $_.Name -eq "ImportExcel" }
If ($latest.version -gt $current.version) {
    Try {
        Update-Module -Name 'ImportExcel' -RequiredVersion $latest.version -AllowPrerelease
        Write-Host "ImportExcel PowerShell module updated successfully to" $latest.Version -ForegroundColor 'Green'
    }
    Catch {
        Write-Host "Unable to update ImportExcel PowerShell module" -ForegroundColor 'Red'
    }
}
Elseif ($null -eq $current.version) {
    Try {
        Write-Host "The ImportExcel PowerShell module is not installed. Hence, installing it..." -ForegroundColor "Yellow"
        Install-Module 'ImportExcel' -Scope CurrentUser -force
    }
    Catch {
        Write-Host "Unable to install the ImportExcel PowerShell module"
    }

}
Else {
    Write-Host "`nLatest version of ImportExcel is not newer than the current" -ForegroundColor 'Yellow'
}
