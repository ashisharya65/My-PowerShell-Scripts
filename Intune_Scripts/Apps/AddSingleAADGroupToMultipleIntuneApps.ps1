<#
    .SYNOPSIS
        This script will add single Azure AD (AAD) group to multiple Intune Apps.
    .DESCRIPTION
        With this script, you will be able to add single AAD group to multiple Intune apps along with choosing the Install mode (Available or Required)
    .NOTES
        Author: Ashish Arya
        Date: 18 Jan 2023
...
#Prompts for Application, InstallMode and Azure AD Group
$AADGroup = Read-Host -Prompt "Enter the group name"
$TargetGroupId = (Get-AADGroup -GroupName $AADGroup).id
$Installmode = Read-Host -Prompt "Enter the Installation mode (Available or Required)"
$IntuneApps = Get-Content (Read-Host -prompt "Enter path of the text file which stores Intune Application names")

#Looping through all intune apps in order to add the concerned Azure Ad group to them.
Foreach ($IntuneApp in $IntuneApps) {
    ...
    $Application = Get-IntuneApplication | where-object { $_.displayName -eq "$IntuneApp" }
